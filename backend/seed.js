...
mongoose
  .connect('mongodb://localhost:27017/trackex', {
  ...
  .then(async x => {
      ...
      .map(transaction => {
        if (transaction.category.value && transaction.type.value) {
          return {
            ...
            category: transaction.category.value,
            type: transaction.type.value,
            userId: mongoose.Types.ObjectId('61b23bc91e8d687234f55d43'),
          }
        }
      })
      .filter(x => x)
    try {
      ...
      console.log(
        `Success! - ${createdTransactions.length} transactions created correctly for user letiapp5@gmail.com`
      )
